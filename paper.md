...
Ultimately, a much better process would be if specimens and sequence were linked stably from their creation. This would require both entities to have persistent identifiers and that those identifiers could, if necessary, be retrospectively added to each record. Various things stand in the way of making this a reality. For example, there is a long lag time between a specimen entering a collection and it being accessioned with an identifier and also, scientist documenting specimens are poorly trained, making them unaware of appropriate standards. Part of the problem is that publications are what are used to evaluate scientists and the data underlying these papers are rarely peer reviewed with the same rigor. Nor are there appropriate incentives to motivate scientists to care for their legacy data [@10.1016/j.tree.2013.05.002].  
## 4.2 Recommendations
